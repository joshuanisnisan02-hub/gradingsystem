...
class _GradebookScreenState extends State<GradebookScreen> {
  ...
  Future<_MasterlistImport?> _pickStudentMasterlist() async {
    ...
    final headers=rows.first.map((cell)=>cell.toString().replaceFirst('\ufeff','').trim().toLowerCase().replaceAll(RegExp(r'[^a-z0-9]+'),'_').replaceAll(RegExp(r'^_|_$'),'')).toList();
    ...
    for(final row in rows.skip(1)){
      ...
      if((lastName.isEmpty||firstName.isEmpty)&&nameColumn>=0){
        ...
        if(comma>0){lastName=combinedName.substring(0,comma).trim();firstName=combinedName.substring(comma+1).trim();}
  ...
  Future<void> importStudentMasterlist() async {
    try{
      ...
      for(final student in imported.students){
        ...
        if(!existing.containsKey(number)){final id=uuid.v4();existing[number]=id;newStudents.add({'id':id,...student});}
      }
