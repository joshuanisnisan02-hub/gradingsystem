...
class _GradebookScreenState extends State<GradebookScreen> {
  ...
  Future<_MasterlistImport?> _pickStudentMasterlist() async {
    ...
    final headers=rows.first.map((cell)=>cell.toString().replaceFirst('\ufeff','').trim().toLowerCase().replaceAll(RegExp(r'[^a-z0-9]+'),'_').replaceAll(RegExp(r'^_|_
    try{
    ...
    child: loading ? const Center(child: CircularProgressIndicator()) : Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Wrap(runSpacing: 12, spacing: 12, crossAxisAlignment: WrapCrossAlignment.center, children: [
          ...
          OutlinedButton.icon(onPressed:saving?null:importStudentMasterlist,icon:const Icon(Icons.group_add_outlined,size:17),label:const Text('Import masterlist')),
...
class _MasterlistImport {
  const _MasterlistImport(this.fileName,this.students);
...
class _OverallEmpty extends StatelessWidget{
  ...
  @override Widget build(BuildContext context)=>Container(width:double.infinity,decoration:BoxDecoration(color:Colors.white,border:Border.all(color:SmartGradeColors.line),borderRadius:BorderRadius.circular(9)),child:const Column(mainAxisAlignment:MainAxisAlignment.center,children:[Icon(Icons.calculate_outlined,size:43,color:SmartGradeColors.red),SizedBox(height:12),Text('No scores to summarize yet',style:TextStyle(fontWeight:FontWeight.w800,fontSize:17)),SizedBox(height:5),Text('Add items under the five grading categories first.',style:TextStyle(color:SmartGradeColors.muted))]));
    ...
    String value(List<dynamic> row,int index)=>index>=0&&index<row.length?row[index].toString().trim():'';
    ...
    for(final row in rows.skip(1)){
      ...
      if((lastName.isEmpty||firstName.isEmpty)&&nameColumn>=0){
        ...
        if(comma>0){
          ...
          if(parts.length>1){lastName=parts.removeLast();firstName=parts.join(' ');}
        }
      }
    ...
    return _MasterlistImport(file.name,students);
  }

  Future<void> importStudentMasterlist() async {
    try{
      final imported=await _pickStudentMasterlist();
      if(imported==null||!mounted)return;
      final currentNumbers=enrollments.map((enrollment)=>(enrollment['students'] as Map<String,dynamic>)['student_number'].toString().toLowerCase()).toSet();
      final newCount=imported.students.where((student)=>!currentNumbers.contains(student['student_number'].toString().toLowerCase())).length;
      final approved=await showDialog<bool>(context:context,builder:(context)=>AlertDialog(
        title:const Text('Import student masterlist?'),
      ...
      setState(()=>saving=true);
      final numbers=imported.students.map((student)=>student['student_number'] as String).toList();
      final existingData=await supabase.from('students').select('id, student_number').inFilter('student_number',numbers);
      final existing=<String,String>{for(final student in existingData)student['student_number'].toString():student['id'].toString()};
      const uuid=Uuid();
      final newStudents=<Map<String,dynamic>>[];
      for(final student in imported.students){
        final number=student['student_number'] as String;
        if(!existing.containsKey(number)){
          final id=uuid.v4();
          existing[number]=id;
          newStudents.add({'id':id,...student});
        }
      }
      ...
      if(mounted)ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text('Student masterlist could not be imported: '+e.toString()),duration:const Duration(seconds:8)));
    }finally{if(mounted)setState(()=>saving=false);}
  }
